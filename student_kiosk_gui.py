...
class CloudSyncWorker(threading.Thread):
    def __init__(self, sqlite_db_path="novalunch_edge.db", sync_interval_sec=10):
        super().__init__()
        self.daemon = True
...
def get_yolo_model(pt_path="novalunch_yolo.pt"):
    ...
    def transition_to_state(self, new_state):
        ...
        if new_state == STATE_IDLE:
            ...
            try:
                ...
                os.makedirs("trays_queue", exist_ok=True)
                timestamp = int(time.time())
                student_id = self.active_student["id"] if self.active_student else "GUEST"
                filename = f"trays_queue/tray_{timestamp}_{student_id}.jpg"
