...
if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
        ...
        const rememberMe = document.getElementById('rememberMeCheck')?.checked ? '1' : '0';
        ...
        loginSubmitBtn.querySelector('.btn-label').textContent = 'Verifying…';

        const formData = new FormData();
        formData.append('email',       email);
        formData.append('password',    password);
        formData.append('remember_me', rememberMe); // ← new
        ...
        try {
            ...
            if (data.success) {
                if (data.otp_required) {
                    // ── New / unrecognised device: show OTP modal ──
                    showOtpModal();
                } else {
                    // ── Trusted device: skip OTP, go straight in ──
                    if (data.trusted_device) {
                        showToast('Recognised device — logging you in…', 'success');
                    }
            ...
            loginSubmitBtn.querySelector('.btn-label').textContent = 'Access Portal';
        }
    });
}
