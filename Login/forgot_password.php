 ...
 * ─────────────────────────────────────────────────────────
 * Location: C:\xampp\htdocs\Login\forgot_password.php
 *           (same folder as login.php, logindb.php, Maininterface.html)
 *
 * Called by Maininterface.js: fetch('forgot_password.php', ...)
 *
 * Flow:
 *   1. Validate email
 *   2. Look up user in school_system.users
 *   3. Generate secure token → store SHA-256 hash in password_reset_tokens
 *   4. Send branded reset-link email via PHPMailer (matches login.php style)
...
require_once 'logindb.php'; // $conn (PDO) — same folder as login.php

// ── PHPMailer — same paths as login.php ──────────────────
...
// ── 2. Look up user ───────────────────────────────────────
// Checks email, school_email, personal_email — mirrors login.php OTP target logic
try {
    $stmt = $conn->prepare(
    ...
    error_log('[forgot_password] DB lookup: ' . $e->getMessage());
    respond(['success' => false, 'message' => 'A database error occurred. Please try again.']);
}

// ── 3. Generate token & send email (only if user found) ───
if ($user) {

    // Send to personal_email if valid — same logic as login.php OTP
    $personal = trim($user['personal_email'] ?? '');
    $sendTo   = filter_var($personal, FILTER_VALIDATE_EMAIL) ? $personal : $user['email'];

    // Raw token in the email link — only the hash is stored in DB
    $rawToken  = bin2hex(random_bytes(32));
    $tokenHash = hash('sha256', $rawToken);
    $expiresAt = date('Y-m-d H:i:s', strtotime('+10 years')); // No practical expiry — token is invalidated by used_at on use

    // ── Store token in DB ─────────────────────────────────
    try {
        $conn->prepare("DELETE FROM password_reset_tokens WHERE user_id = ?")
             ->execute([$user['id']]);
        ...
        error_log('[forgot_password] Token store: ' . $e->getMessage());
        respond(['success' => false, 'message' => 'Could not create reset token. Please try again.']);
    }

    // ── Build reset URL ───────────────────────────────────
    // reset_password.php is in the same folder as this file
    $scheme   = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host     = $_SERVER['HTTP_HOST'];
    $dir      = rtrim(dirname($_SERVER['SCRIPT_NAME']), '/\\');
    $resetUrl = $scheme . '://' . $host . $dir . '/reset_password.php?token=' . urlencode($rawToken);

    // ── Send via PHPMailer ────────────────────────────────
    $roleLabel = ucfirst(str_replace('_', ' ', $user['role']));
    $year      = date('Y');
    $logoSrc   = 'https://i.imgur.com/kR21xJw.png'; // same hosted logo as login.php
    ...
    try {
        ...
        $mail->AltBody  = "Reset your SJC Portal password:\n{$resetUrl}\n\nThis link is valid until you use it — it can only be used once.\nIf you did not request this, ignore this email.";

        $mail->send();

    } catch (MailException $e) {
        error_log('[forgot_password] Mail: ' . $mail->ErrorInfo);
        // Fall through — never reveal mail failure to prevent enumeration
    }
}

// ── 4. Always generic success ─────────────────────────────
respond([
    'success' => true,
...
// ═════════════════════════════════════════════════════════
//  EMAIL HTML — identical design to login.php OTP email
//  · Maroon header (#1a0000) · Gold accents (#c9a84c)
//  · School logo · Inline styles only
// ═════════════════════════════════════════════════════════
function buildResetEmailHtml(string $resetUrl, string $roleLabel, string $year, string $logoSrc): string
{
    $logoTag = $logoSrc
        ...
        <tr>
          <td style="padding:40px 36px 36px;">
            <p style="margin:0 0 6px;font-family:Georgia,'Times New Roman',serif;font-size:22px;
                       color:#1a0000;font-weight:normal;line-height:1.3;">Reset Your Password</p>
            <p style="margin:0 0 28px;font-family:Arial,sans-serif;font-size:13px;color:#6b5f55;line-height:1.7;">
            ...
            <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 auto 32px;">
              <tr>
                <td style="border-radius:8px;background:#1a0000;">
                  <a href="{$safeUrl}" style="display:inline-block;padding:16px 44px;font-family:Arial,sans-serif;
                             font-size:14px;font-weight:700;letter-spacing:1px;color:#c9a84c;text-decoration:none;
              ...
              <tr>
                <td style="padding:16px 20px;">
                  <p style="margin:0 0 6px;font-family:Arial,sans-serif;font-size:11px;font-weight:700;
                             color:#1a0000;letter-spacing:1px;text-transform:uppercase;">Button not working?</p>
                  <p style="margin:0 0 8px;font-family:Arial,sans-serif;font-size:12px;color:#5a4e46;line-height:1.6;">
                    Copy and paste this link into your browser:</p>
                  <p style="margin:0;font-family:'Courier New',monospace;font-size:11px;color:#3a6fa8;
                             word-break:break-all;line-height:1.6;">{$safeUrl}</p>
                </td>
              </tr>
            </table>
              ...
              <tr>
                <td style="padding:16px 20px;">
                  <p style="margin:0;font-family:Arial,sans-serif;font-size:12px;color:#7a2020;line-height:1.7;">
                    ...
                    your password will <em>not</em> change.
                    Never share this link with anyone. SJC staff will <em>never</em> ask for your reset link.
                  </p>
                </td>
              </tr>
            </table>
